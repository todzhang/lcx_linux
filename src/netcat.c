...
char *winsockstr(error) int error;
{
    switch (error)
    {
    case WSAEINTR:
...
void arm(num, secs) unsigned int num;
unsigned int secs;
{
...
char *Hmalloc(size) unsigned int size;
{
...
unsigned int findline(buf, siz) char *buf;
unsigned int siz;
{
    register char *p;
    register int x;
...
HINF *gethostpoop(name, numeric) char *name;
USHORT numeric;
{
...
USHORT getportpoop(pstring, pnum) char *pstring;
unsigned int pnum;
{
...
USHORT nextport(block) char *block;
{
...
recycle:
    ...
    while (argv[optind])
    {
            ...
            while (loport <= curport)
        {
            ...
            Debug(("netfd %d from port %d to port %d", netfd, ourport, curport)) if (netfd > 0) if (o_zero && o_udpmode) /* if UDP scanning... */
