...
void ssocksd(GlobalArgs args)
{
  ...
  while (boucle_princ)
  {
    ...
    {
      if (errno == EINTR)
        ; /* Received signal, it does nothing */
      else
      {
        perror("select");
        goto fin_serveur;
  ...
  *maxfd = soc_ec_cli;
  for (nc = 0; nc < MAXCLI; nc++)
  {
    ...
    if (client->soc_bind.soc != -1)
    {
      ...
      if (client->soc_bind.soc > *maxfd)
        *maxfd = client->soc_bind.soc;
    }
  }
}
